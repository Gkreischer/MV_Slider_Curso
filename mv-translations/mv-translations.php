...
if (!class_exists('MV_Translations')) {
    ...
    class MV_Translations
    {
        ...
        public static function activate()
        {
            ...
            $table_name = $wpdb->prefix . 'translationsmeta';

            // Verifica se a tabela customizada já existe
            ...
            if (empty($mvt_db_version)) {
                ...
                dbDelta($query);
                $mvt_db_version = '1.0';
                // Cria um registro na wp-options para armazenar a versão do plugin
                add_option('mv_translation_db_version', $mvt_db_version);
            }

            // Ao ativar o plugin, cria a página abaixo com o shortcode
            if($wpdb->get_row("SELECT post_name FROM {$wpdb->prefix}posts WHERE post_name = 'submit-translation'") === null) {
                ...
                $page = [
                    ...
                    'post_type' => 'page',
                    'post_content' => '<!-- wp:shortcode -->[mv_translation]<!-- /wp:shortcode -->',
                ];
            ...
            if($wpdb->get_row("SELECT post_name FROM {$wpdb->prefix}posts WHERE post_name = 'edit-translation'") === null) {
                ...
                $page = [
                    ...
                    'post_author' => $current_user->ID,
                    'post_type' => 'page',
                    'post_content' => '<!-- wp:shortcode -->[mv_translation_edit]<!-- /wp:shortcode -->',
                ];
        ...
        {
            flush_rewrite_rules();
            unregister_post_Type('mv_translation');
        }
