...
function mvt_register_user(){
    ...
    if( isset( $_POST['submitted'] )){
        ...
        if(empty($username) || empty($firstname) || empty($lastname) || empty($useremail) || empty($password)){
            $reg_errors->add('empty-fields', esc_html__( 'Required form field is missing', 'mv-translations' ));
        }

        if(strlen($username) < 6) {
            $reg_errors->add('username-length', esc_html__( 'Username must be at least 6 characters long', 'mv-translations' ));
        }

        if(username_exists($username)){
            $reg_errors->add('username-exists', esc_html__( 'Invalid credentials', 'mv-translations' ));
        }

        if(!validate_username($username)){
            $reg_errors->add('username-invalid', esc_html__( 'Invalid credentials', 'mv-translations' ));
        }

        if(!is_email($useremail)){
            $reg_errors->add('username-invalid', esc_html__( 'Email is not valid', 'mv-translations' ));
        }
        ...
        if(strlen($password) <= 5 ) {
            $reg_errors->add('password-length', esc_html__( 'Password must be at least 6 characters long', 'mv-translations' ));
        }

        if(is_wp_error($reg_errors)){
            foreach($reg_errors->get_error_messages() as $error){
                ?>
                    <div style="color: red; text-align: left;"><?php echo $error; ?></div>
                <?php
            }
        }
